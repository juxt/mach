...
maps which describe when and how to build the target). If you use a map,
you can put anything you like in this map, but keys with the `mach`
...
As an alternative to `update!`, a recipe can declare a `produce` entry.
...
declared in a recipe and referenced from other parts of the Machfile,
...
For recipes that have a `produce`, this is called and output is sent to
